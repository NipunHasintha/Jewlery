...
import com.project.studiomanagementsystem.constant.CommonConstant;
...
public class CommonValidation {
    ...
    public static boolean urlValidation(String urlStr) {
        Pattern p = Pattern.compile(CommonConstant.REGEX_URL_VALIDATION);
    ...
    public static boolean passwordValidation(String passwordStr) {
        if (passwordStr.matches(CommonConstant.REGEX_PASSWORD_VALIDATION))
            return false;
        return true;
    }
    ...
    public static boolean emailValidation(String emailStr) {
        Pattern p = Pattern.compile(CommonConstant.REGEX_EMAIL_VALIDATION);
    ...
    public static boolean validNic(String nic){
        if(nic.matches(CommonConstant.REGEX_NIC1_VALIDATION) || nic.matches(CommonConstant.REGEX_NIC2_VALIDATION))
            return false;
        return true;
    }
    ...
    public static boolean isValidMobile(String mobileNum){
        if(mobileNum.matches(CommonConstant.REGEX_MOBILE_VALIDATION)) {
            return false;
        }
        else if (mobileNum.matches(CommonConstant.REGEX_TELEPHONE_VALIDATION)){
            return false;
        }
        return true;
    }
    ...
    public static boolean isCharOnly(String strChar){
        Pattern p = Pattern.compile(CommonConstant.REGEX_CHAR_VALIDATION);
